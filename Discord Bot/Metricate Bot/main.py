...
client =  discord.Client(intents=intents, activity=discord.Activity(type=discord.ActivityType.watching, name="Metricate's server | m!help"))
...
@client.event
async def on_message(message):
  if message.content.startswith('m!pack'):
  ...
  if message.content.startswith('m!help'):
    ...
    embed.set_thumbnail(url="https://yt3.ggpht.com/ytc/AKedOLR2XRSKFE01okbKZGA_x9RIIbAybU8ZlLjvymeNrg=s900-c-k-c0x00ffffff-no-rj")
    embed.add_field(name="m!help", value="Shows this page", inline=True)
    embed.add_field(name="m!faq", value="Shows the most frequently asked questions", inline=True)
    embed.add_field(name="m!credits", value="Shows the developer", inline=True)
    embed.add_field(name="m!pack", value="Shows what texture pack Metricate uses", inline=True)
    embed.set_footer(text="Developed By Wadairity#0706")
    await message.channel.send(embed=embed)
